...
#############################################################
package Allied::Sites::Passports::Filter::Matrix2;
#############################################################
...
sub _init {
        my ($self, $page, @arg) = @_;
	$self->Allied::Sites::Passports::Filter::Matrix::_init($page);
        $self->{FILTER} = 'Matrix2';
        $self->{CUSTOM_FILES_PATH} = '/usr/local/apache/files/Passports/instantpassport_com/';		# path to custom webiste files
	$self->{CONFIG_FILE} = '/usr/local/apache/sites/allied/config/test2.conf';
